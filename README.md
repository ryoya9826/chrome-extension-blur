...
本拡張機能は安全にお使いいただけるよう、以下の設計に基づいています。

- **プライバシー保護**: すべてのフィルタ処理と設定の保存はブラウザ内（ローカル）で完結します。外部のサーバーへユーザーのデータを送信することは一切ありません。
- **安全な処理**: CSS Injectionなどの攻撃を防ぐため、厳格な入力検証と Manifest V3 に準拠した強固なセキュリティポリシー（CSP）を適用しています。
